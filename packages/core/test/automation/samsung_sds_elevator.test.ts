...
describe('Samsung SDS Elevator Automation', () => {
    ...
    describe('Old Style Protocol (4-byte packets)', () => {
        const config: HomenetBridgeConfig = {
            ...
            switch: [
                { id: 'elevator_call', name: 'Elevator Call', type: 'switch', optimistic: true, state: { data: [] } }
            ],
            automation: [
                {
        ...
        beforeEach(() => {
            ...
            automationManager = new AutomationManager(config, packetProcessor as any, commandManager as any, mqttPublisher as any, undefined, mockSender);
        ...
        it('장치 스캔 패킷(AD 5A 00 77) 수신 시 응답 패킷(B0 5A 00 6A)을 전송해야 한다', async () => {
            ...
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x5A, 0x00, 0x6A], expect.anything());
        });
        ...
        it('호출 중이지 않을 때(AD 41 00 6C) 기본 패킷(B0 41 00 71)을 전송해야 한다', async () => {
            ...
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x41, 0x00, 0x71], expect.anything());
        });
        ...
        it('호출 중일 때(AD 41 00 6C) 호출됨 패킷(B0 2F 01 1E)을 전송해야 한다', async () => {
            ...
            packetProcessor.emit('packet', [0xAD, 0x41, 0x00, 0x6C]);
            await vi.advanceTimersByTimeAsync(1);
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x2F, 0x01, 0x1E], expect.anything());
        });
    });
    ...
    describe('New Style Protocol (5-byte packets)', () => {
        ...
        it('장치 스캔 패킷(CC 5A 01 00 17) 수신 시 응답 패킷(B0 5A 01 00 6B)을 전송해야 한다', async () => {
            ...
            await vi.runAllTimersAsync();
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x5A, 0x01, 0x00, 0x6B], expect.anything());
        });
        ...
        it('호출 중이지 않을 때(CC 41 01 00 0C) 기본 패킷(B0 41 01 00 70)을 전송해야 한다', async () => {
            ...
            await vi.advanceTimersByTimeAsync(1);
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x41, 0x01, 0x00, 0x70], expect.anything());
        });
        ...
        it('호출 중일 때(CC 41 01 00 0C) 호출됨 패킷(B0 10 01 01 20)을 전송해야 한다', async () => {
            ...
            packetProcessor.emit('packet', [0xCC, 0x41, 0x01, 0x00, 0x0C]);
            await vi.advanceTimersByTimeAsync(1);
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x10, 0x01, 0x01, 0x20], expect.anything());
        });
        ...
        it('호출 완료 패킷(CC 10 01 01 5C) 수신 시 응답 패킷 전송 및 스위치를 꺼야 한다', async () => {
            ...
            await vi.runAllTimersAsync();
            expect(mockSender).toHaveBeenCalledWith(undefined, [0xB0, 0x41, 0x01, 0x00, 0x70], expect.anything());
            expect(packetProcessor.constructCommandPacket).toHaveBeenCalledWith(expect.objectContaining({ id: 'elevator_call' }), 'command_off', undefined);
        });
    });
});
