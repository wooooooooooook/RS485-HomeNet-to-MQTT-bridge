...
describe('HomeNet to MQTT - CVNet Protocol', () => {
  ...
  it('should generate command packets for all entities', async () => {
    ...
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x11, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0xaa])
    );
    await executeCommand(ctx, 'room_0_light_1', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x53, 0xaa])
    );

    // Room 0 Light 2
    await executeCommand(ctx, 'room_0_light_2', 'on', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x12, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x55, 0xaa])
    );
    await executeCommand(ctx, 'room_0_light_2', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0xaa])
    );

    // Room 0 Light 3
    await executeCommand(ctx, 'room_0_light_3', 'on', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x13, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x56, 0xaa])
    );
    await executeCommand(ctx, 'room_0_light_3', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x21, 0x01, 0x13, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x55, 0xaa])
    );

    // Room 1 Light 1
    await executeCommand(ctx, 'room_1_light_1', 'on', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x22, 0x01, 0x11, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x55, 0xaa])
    );
    await executeCommand(ctx, 'room_1_light_1', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x22, 0x01, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0xaa])
    );

    // --- Fan ---
    await executeCommand(ctx, 'fan_1', 'on', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x71, 0x01, 0x11, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa6, 0xaa])
    );
    await executeCommand(ctx, 'fan_1', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x71, 0x01, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa3, 0xaa])
    );
    ...
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x11, 0x01, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x43, 0xaa])
    );
    ...
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x01, 0x81, 0x81, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x25, 0xaa])
    );
    ...
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x41, 0x01, 0x11, 0x99, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0c, 0xaa])
    );

    // Test command_off (default temp 0)
    await executeCommand(ctx, 'heater_1', 'off', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x41, 0x01, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x73, 0xaa])
    );

    // Test command_heat (default temp 0 -> 0+0x80 = 0x80)
    await executeCommand(ctx, 'heater_1', 'heat', null);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x41, 0x01, 0x11, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf3, 0xaa])
    );

    // Heater 2 (Address 0x42)
    await executeCommand(ctx, 'heater_2', 'temperature', 20);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x42, 0x01, 0x11, 0x94, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0xaa])
    );

    // Heater 3 (Address 0x43)
    await executeCommand(ctx, 'heater_3', 'temperature', 30);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x43, 0x01, 0x11, 0x9e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x13, 0xaa])
    );

    // Heater 4 (Address 0x44)
    await executeCommand(ctx, 'heater_4', 'temperature', 10);
    expect(ctx.mockSerialPort.write).toHaveBeenLastCalledWith(
      Buffer.from([0xf7, 0x20, 0x44, 0x01, 0x11, 0x8a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa])
    );
