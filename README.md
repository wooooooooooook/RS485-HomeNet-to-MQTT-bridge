...
| `MQTT_TOPIC_PREFIX` | 모든 상태/명령 토픽 접두사 | `homenet` |
...
`SERIAL_PORTS`와 `CONFIG_FILES`의 순서는 1:1로 대응하며, 지정된 포트 순서대로 설정 파일을 사용할 때 Home Assistant 토픽 접두사/포트 매핑을 명확히 관리할 수 있습니다. 단일 값 입력은 기존 변수(`SERIAL_PORT`, `CONFIG_FILE`)를 통해서도 유지되지만 향후 복수 포트를 대비해 배열 형식을 권장합니다.
